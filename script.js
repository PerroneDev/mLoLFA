...
function randomizar() {
  ...
  obterDadosItens().then((itens) => {
    ...
    const botas = itens.filter(
      (item) =>
        item.name.includes("Boots of Swiftness") ||
        item.name.includes("Berserker's Greaver") ||
        item.name.includes("Ionian Boots of Lucidity") ||
        item.name.includes("Mercury's Treads") ||
        item.name.includes("Mobility Boots") ||
        item.name.includes("Plated Steelcaps") ||
        item.name.includes("Sorcerer's Shoes")
    );

    const indexBota = Math.floor(Math.random() * botas.length);
    const botaFinal = botas[indexBota];
    console.log(botaFinal);

    const elBota = document.getElementById("bota");
    elBota.style.backgroundImage = `url('https://ddragon.leagueoflegends.com/cdn/13.17.1/img/item/${botaFinal.image.full}')`;
  });
}
